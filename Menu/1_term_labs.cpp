...
void lab2_1()
{
	...
	cout << "\nРазвлетвленный алгоритм с выбором формул\n" << endl;
	cout << "\nВведите данные в виде трех целых чисел и вычислите минимальное и максимальное число, а также их индексы,\
	\nвычислите среднее значение трех чисел, а также отклонение и соотношение трех чисел от среднего:\n" << endl;
	...
	cout << "Отклонение и соотношение b1 от среднего: " << round(d1 * 100) / 100 << ", " << round(d1_c * 100) / 100 << "%" << endl;
	float d2 = abs(b2 - bmid); float d2_c = d2 / (bmax - bmin) * 100;
	cout << "Отклонение и соотношение b2 от среднего: " << round(d2 * 100) / 100 << ", " << round(d2_c * 100) / 100 << "%" << endl;
	float d3 = abs(b3 - bmid); float d3_c = d3 / (bmax - bmin) * 100;
	cout << "Отклонение и соотношение b3 от среднего: " << round(d3 * 100) / 100 << ", " << round(d3_c * 100) / 100 << "%" << endl;
	_getch();
}
void lab2_2()
{
	...
	cout << "\nРазвлетвленный алгоритм с анализом исходных данных и принтием решения\n";
	cout << "\nЕсть ли среди трех чисел такое, которое равно квадрату разности двух других.\nЕсли да, то выведите это число\n";
	while (a == 1) { // цикл повтора запуска программы
		cout << "\nВведите данные в виде трех целых чисел:\n";
		cout << "\nВведите первое число: ";
		cin >> C;
		cout << "\nВведите второе число: ";
		cin >> F;
		...
		//вывод ответа в зависимости от условия
			...
			cout << "Нет удовлетворяющих условию чисел" << endl;
		}
...
void lab3_1()
{
	cout << "Циклический алгоритм с несколькими параметрами и проверкой условий\n\n";
	cout << "Выведите в столбце N значений функции:\n\n";
	cout << "-sqrt(|16 * a^2 + 4 * a^2 * (x + a)^2)|) - (x+a)^2 - a^2, если x < k * a\n";
	cout << " sqrt(|a^2 - (x + a^2)|) - 2 * a, если x >= k*a\n";
...
void lab3_2()
{
	...
	cout << "\nВведите данные массива A в виде целых чисел для определения нового массива B,\nгде Bi = sum(A) + 2*Ai если Ai > 2,\nв противном случае Bi = sum(A) + 3*Ai, \
где sum(A) - сумма элементов нового массива:\n\n " << endl;
	cout << "условие для вывода: каждый выведенный элемент должен быть меньше среднего суммы максимального и минимального элемента массива\n\n";
	sum = 0;
	for (i = 0;i < 12;i++) {
		do { //проверка на равенство значения нулю
			cout << "\nВведите ненулевое значение" << i + 1 << "/12 число" << endl;
			cin >> y[i];
		} while (y[i] == 0);
	...
	cout << endl << "\nКоличество чисел, подчиняющиеся условиям: " << l;
	_getch();
}
void lab4()
{
	const int ROW = 6, COL = 6;
	cout << "Циклический алагоритм обработки матрицы\n(Если произведение элементов четного столбца меньше или равно 0, то такой столбец заполняется нулями) " << endl;
